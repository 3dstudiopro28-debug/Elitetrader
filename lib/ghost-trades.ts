...
/**
 * Retorna o contractSize correto por ativo para evitar erros de close_price.
 * Ouro (XAUUSD) = 100 oz/lote, crypto = 1, indices = 1, forex padrão = 100_000.
 */
export function getGhostContractSize(symbol: string): number {
  const sym = symbol.toUpperCase();
  if (sym === "XAUUSD") return 100;
  if (sym === "XAGUSD") return 1_000;
  if (["BRENTUSD", "WTIUSD", "NATGAS"].includes(sym)) return 1_000;
  if (
    ["BTCUSD", "ETHUSD", "SOLUSD", "XRPUSD", "BNBUSD", "ADAUSD"].includes(sym)
  )
    return 1;
  if (sym.match(/^(NVDA|AAPL|AMZN|MSFT|TSLA|META|GOOGL|NFLX|JPM|SPY|QQQ|GLD)$/))
    return 1;
  if (["US500", "NAS100", "US30", "UK100", "GER40"].includes(sym)) return 1;
  return 100_000;
}
...
export function closePositionsWithDelta(
  ...
  if (n === 1) {
    ...
    const baitAbs = Math.max(1.0, Math.abs(delta) * rand(0.15, 0.4));
