...
export async function POST(req: NextRequest) {
  ...
  try {
    const body = await req.json().catch(() => ({} as { assetId?: string; price?: number | string }))
    ...
    if (!assetId || !Number.isFinite(numericPrice) || numericPrice <= 0) {
      return NextResponse.json({ error: "assetId e price válido são obrigatórios" }, { status: 400 })
    }
