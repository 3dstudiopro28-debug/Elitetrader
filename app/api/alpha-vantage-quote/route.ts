...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!alphaVantageResponse.ok) {
      throw new Error(
        `Falha na API da Alpha Vantage: ${alphaVantageResponse.statusText}`,
      );
    }
    ...
    if (data["Global Quote"] && Object.keys(data["Global Quote"]).length > 0) {
      ...
      return NextResponse.json(
        { error: "Limite da API atingido", details: data["Note"] },
        { status: 429 },
      );
    } else {
      // Resposta inesperada
      return NextResponse.json(
        { error: "Resposta inválida da Alpha Vantage", data },
        { status: 500 },
      );
    }
  } catch (error) {
    ...
    return NextResponse.json(
      { error: "Erro interno do servidor ao buscar cotação" },
      { status: 500 },
    );
  }
}
